...
{    int ** copy=new int * [vertex];
        for(int i=0;i<vertex;i++)
    {   copy[i]=new int[vertex];
        for(int j=0;j<vertex;j++)
            copy[i][j]=adj[i][j];
    }
    ...
    dfs(s,parent,visited);
      ...
      while(parent[d]!=-1)
       ...
       for(int i=0;i<vertex;i++)
            for(int j=0;j<vertex;j++)
                {
                    //cout<<"adj[i][j] == "<<adj[i][j]<<" "<<visited[i]<<" "<<visited[j]<<endl;

                if(copy[i][j]>0&&visited[i]==1&&visited[j]==0)
                {
                    cout<<i<<"-"<<j<<" belongs to min cut"<<endl;
                }
