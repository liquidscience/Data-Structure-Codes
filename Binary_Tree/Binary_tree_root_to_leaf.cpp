...
void root_to_leaf(struct node * root,int display[],int on)
{

    if(root!=NULL)
    display[on++]=root->data;
    else
        return;
    ...
    else
    {
        ...
        if(root->left_child)
            root_to_leaf(root->left_child,display,on);
        if(root->right_child)
            root_to_leaf(root->right_child,display,on);
